...
- 成员考勤：法定工作日默认正常出勤，只需登记加班、调休或特殊正常出勤；按项目状态历史逐日计算，仅将某日处于 `准备 / 进行 / 维护` 的参与项目标记在该日考勤中，并提供月度汇总和异常明细。
- 项目管理：支持 `准备 / 进行 / 维护 / 暂停 / 结束`，每个项目使用不可修改的纯数字项目 ID，并完整记录状态变化和成员加入/退出历史。
- 每日进展：按项目、日期记录工作内容，可在跨项目日期矩阵中查看。
- AI 日报：使用 AstrBot 已接入的聊天模型，从推进中或暂停的项目中按日生成总结；推进中项目默认勾选，结束项目不显示。可在项目进程相同的时间范围内查看生成状态，每天只保存一份，并支持预览覆盖、人工修订和删除。
...
`项目进程` 的项目 ID 后全部内容都会作为当天记录保存，支持空格和换行。`项目总结` 的第一个参数是附带前几天总结的天数（0～30），后面可选项目 ID；不填写项目 ID 时使用生成总结页面默认勾选的准备、进行、维护项目，并直接覆盖当天已有总结。
...
- 当前已注册跨平台消息指令 `项目总览`、`项目进程`、`项目总结`，由 AstrBot 的消息事件过滤器统一处理。
